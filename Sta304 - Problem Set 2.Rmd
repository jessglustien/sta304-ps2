...
data1 <- gss %>% select(income_family, ever_married, education, total_children, age_at_first_birth)%>%filter(!is.na(total_children), !is.na(age_at_first_birth))

df %>% ggplot(aes(x = age_at_first_birth)) + geom_histogram(bins = 5) + labs(x = "age at first birth", title ="Figure 1")

#discussion

df %>% ggplot(aes(y = age_at_first_birth, x = total_children)) +geom_point() + labs(x = "total children", title ="Figure 2")

#discussion

mod <- lm(age_at_first_birth~ income_family + ever_married + education + total_children, data = data1)
